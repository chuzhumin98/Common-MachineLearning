...
# AdaBoost.M1的更为一般的实现框架
#    methodLists:AdaBoost各时刻所用的方法集
def commonAdaBoost(trainMatrix, trainLabels, testMatrix, methodLists):
    T = len(methodLists)
    weight = np.ones(len(trainLabels))/len(trainLabels) #各样本点的权重，初始时权重相同
    modelWeight = np.zeros([T,1]) #所得到的各个模型的权重
    models = np.zeros([T, len(testMatrix)]) #各个model对测试集数据的分类结果
    for i in range(T):
        ...
        AdaBoostTrainMatrix = trainMatrix[AdaBoostIndex, :]  # 重采样得到的训练样本
        AdaBoostTrainLabels = trainLabels[AdaBoostIndex]  # 重采样得到的训练标签
        predictResults = chooseMethod(AdaBoostTrainMatrix, AdaBoostTrainLabels, np.vstack((trainMatrix, testMatrix)), methodLists[i])
        predictTrainLabels =predictResults[0:len(trainMatrix)]
        models[i, :] = predictResults[len(trainMatrix):]
        deltaPredict = trainLabels - predictTrainLabels  # 将真实标签与测试标签进行作差
        misclassifyWeight = np.minimum(np.abs(deltaPredict),1) #计算偏差的大小，当大于1则认为全部分错
        epsilon = np.sum(np.multiply(weight,misclassifyWeight))
        print(i, ':', epsilon)
        if (epsilon >= 0.5):
            continue  # 当错误率太大时，则丢弃模型，不更新模型权值
        classifyIndex = deltaPredict[:] == 0
...
if __name__ == '__main__':
    ...
    methodLists = [1,0,3,2,5,6,4,7]
    predictResult = commonAdaBoost(trainAppearMatrix0, trainLabels0, testAppearMatrix, methodLists)
    exportResult(predictResult, 'result/AdaBoost_[1,0,3,2,5,6,4,7]_v2.csv')